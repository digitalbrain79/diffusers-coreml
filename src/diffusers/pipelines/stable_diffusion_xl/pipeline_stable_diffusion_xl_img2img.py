    ...
    def prepare_latents(
        ...
        if image.shape[1] == 4:
            ...
            if self.vae_encoder.config.force_upcast:
                image = image.float()
                self.vae_encoder.to(dtype=torch.float32)
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                init_latents = [
                    retrieve_latents(self.vae_encoder.encode(image[i : i + 1]), generator=generator[i])
                    for i in range(batch_size)
                ]
                init_latents = torch.cat(init_latents, dim=0)
            else:
                init_latents = retrieve_latents(self.vae_encoder.encode(image), generator=generator)

            if self.vae_encoder.config.force_upcast:
                self.vae_encoder.to(dtype)
            ...
            if latents_mean is not None and latents_std is not None:
                ...
                init_latents = self.vae_encoder.config.scaling_factor * init_latents
